...
public class Collector extends AbstractRunRiverThread {
    ...
    public void collectStreams() throws Exception {
        ...
        SearchResponse sResponse = remoteClient.prepareSearch(tribeIndex)
                                               ...
                                               .addAggregation(AggregationBuilders.terms("streams").field("stream").size(200)
                                                 .subAggregation(AggregationBuilders.terms("ls").field("ls").size(30).order(Terms.Order.term(false))
    ...
    public void collectStates() throws Exception {
        ...
        SearchResponse sResponse = remoteClient.prepareSearch(tribeIndex)
                                               ...
                                               .setSize(0)
                                               .addAggregation(AggregationBuilders.terms("mclass").field("mclass").size(0).order(Terms.Order.term(true)).minDocCount(1)
                                                 .subAggregation(AggregationBuilders.terms("hmicro").field("micro").size(0).order(Terms.Order.term(true)).minDocCount(1))
                                                 .subAggregation(AggregationBuilders.terms("hinput").field("instate").size(0).order(Terms.Order.term(true)).minDocCount(1))
                                               )
                                               .addAggregation(AggregationBuilders.terms("hmicro").field("micro").size(0).order(Terms.Order.term(true)).minDocCount(1))
                                               .addAggregation(AggregationBuilders.terms("hmini").field("mini").size(0).order(Terms.Order.term(true)).minDocCount(1))
                                               .addAggregation(AggregationBuilders.terms("hmacro").field("macro").size(0).order(Terms.Order.term(true)).minDocCount(1))
                                               .addAggregation(AggregationBuilders.terms("hinput").field("instate").size(0).order(Terms.Order.term(true)).minDocCount(1))
